...
<%
	...
	try{  
		...
		q = st.executeQuery("SELECT a.auction_id, c.name, c.type FROM Auctions AS a JOIN Clothing AS c WHERE a.item_is = c.product_id AND a.is_active = 1 AND a.auction_id IN(SELECT DISTINCT for_auction FROM Bids WHERE from_user = " + request.getSession().getAttribute("user") + " AND a.is_active = 1)");
		while(q.next()){
